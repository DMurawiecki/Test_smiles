...
python solution.py
```
Or you can clone repo to Google Colab and run it with base colab environment


# Solution
In this work I researched hallucination detection task, using Qwen2.5-0.5B hidden states and built a lightweight classifier. I used the initial splitting setup because it is the most reliable way to have a held-out test data to have accurate evaluation and prevent data leakage.

## Experiments

### Layer and token ablation
As was discovered in previous papers (Orgad et al., 2025), the attributes of truthfulness are actually encoded in the internal representations of the model, and this information may be distributed unevenly across layers. Most infromation is encoded in the middle and last layers. So I tried different setups, but the most effective ones is averaging by 21-24 layers. 
...
Also I tried using different last token setups for features such as only 1, last 8, 16, etc. And it was found that the most effective setup is adding long token window, that's why I used (1, 8, 16, 32, 64, 128, 256, 512) as a final setup.
...
### Model
I've tested a lot of models(Logistic Regression, Lasso/Ridge, 2,3 layer MLP, k-NN and SVM) and I figured out that the best one is SVM with linear kernel. It is really lightweight classifier, and it showed better performance than MLP, because MLP can potentially learn a more complex boundary, but is more likely to overfit on a small dataset. 
...
And the final hypothesis is that hallucination may manifest itself not only in a specific hidden state, but also in how the answer representation changes across layers. The idea was really similar to (Hameed Mir et al., 2025).
In my experiments the idea was checked in LSD++ trajectory-features. For different ranges I calculated cues of change in answer representations between layers: norms, differences, cosine similarity between adjacent layers, slope, drift, and stability statistics.
